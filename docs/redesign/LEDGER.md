...
### RC-pass OUTCOMES (delivered)
- **Product-truth reverify (agent):** 0 P0 overclaims. UNDERCLAIM found — valuations/applications-for-payment now fully LIVE (mig 20261192, portal-exposed) → re-added to Money pillar + differentiation + signature "Bill the work". CIS "…& verification" → "…& statements"; softened "in daily use" traction.
- **Real proof expanded to 7 curated screens:** dashboard, cash position, jobs, invoices, customers, quotes, RAMS (seeded 2 issued RAMS + 4 quotes into the Brightwork demo org). Pillars: win-work=customers, run-jobs=jobs, money=cash, site-safety=RAMS.
- **Signature moment rebuilt** as a robust FILMSTRIP (each stage = narrative + a large real product frame on the gold setting-out spine, Reveal-wrapped) after the sticky-scrollytelling proved empty-looking + unverifiable. Mobile keeps its bespoke stacked sequence.
- **Book-demo modal** redesigned to the dark system + real focus trap/restore/scroll-lock/aria (logic untouched); mobile bottom sheet. Legal pages (/privacy /terms) darkened.
- **Adversarial reviewers:** anti-vibecode **91/100** ("genuinely bespoke, not vibecoded") + frontend code/perf **88/100**; a11y + SEO self-audited (agents hit env sleep-errors). Fixed: FABRICATED "22%" invoicing stat (features.ts) + a soft-proof "owners report" line; deleted dead marketing.css/motion.tsx(framer-motion)/cta.tsx + Logo/Dim; container 1280→max-w-cf; white-body overscroll (`html:has([data-marketing])`); de-genericised OutcomeCards → numbered "schedule".
- **Gates:** tsc 0, lint clean, **10,784 tests pass**, CI green, Vercel preview building/green. Hero clean at 320/390/768/1440/1728; 0 dangling internal links; 1 h1/page.

...
**Programme:** Final Live Activation · **Baseline:** main `d2638d56` = prod `d2638d5` · parity 387/387 tip `20261227` · **Date:** 2026-08-30 · **Last updated:** 2026-09-13 — AI estate: **all five governor tiers armed** (cheap/mid/high 2026-09-10 · embedding 2026-09-11 · transcription 2026-09-13, channel-dark); see the proof block
...
|---|---|---|---|
| **Generative AI estate** (~13 surfaces: quote writer, OCR, receptionist drafting, HQ narratives/seams, insights…) | R039 R050 R089 R091 R093 R101 (+R034) | Per-token usage; £100/org/month fail-closed ceiling with atomic pre-reservation | **ARMED 2026-09-10** (CEO-approved reviewed diff): cheap=claude-haiku-4-5-20251001, mid=claude-sonnet-5, high=claude-opus-5; `ANTHROPIC_API_KEY` now reaches the governed doors only (still 0 ungoverned entry points, security-pinned). **Embedding tier ARMED 2026-09-11** (CEO-approved: OpenAI text-embedding-3-small; `memory_embedding.worker_enabled` flag ON in prod since 2026-09-11) and **transcription tier ARMED 2026-09-13** (CEO-approved: OpenAI gpt-4o-mini-transcribe; governor + transport bindings in one reviewed diff, PR #871 `9785c8e`; WhatsApp channel stays dark) — **all five tiers bound**. **CEO HOLD (2026-09-10): portal-chat generative auto-reply stays off** via the `CHAT_AUTO_REPLY_GENERATIVE=false` build constant (server/services/chat-auto-reply.ts) — the only surface that would auto-post unreviewed AI prose to a customer; every other customer-facing generative surface is draft-first/human-review. **LIVE — PRODUCTION-PROVEN (generative trio + embedding); transcription armed, provider self-test pending** — evidence in §AI-estate proof below |
...
6. Marketing-drift flag (2026-09-13, no code change — **pending CEO/marketing decision**): the exclusion list in `lib/marketing/pillars.ts:4-13` now UNDERSELLS the live product — AI insights and the AI quote writer went live with the tier arming; whether to market them is a CEO/marketing call, so the list is deliberately left untouched.

## AI-estate production proof — all tiers (generative trio + embedding; transcription provider-armed) (stamped 2026-09-11, updated 2026-09-13)

Every tier proven by a REAL governed call through the genuine application path in production — no direct provider calls, no weakened controls. Canonical ledger rows (`ai_invocations` ⟷ `ai_cost_reservations`, prod):

| Tier | Feature | Model | Tokens in/out | Reserved → settled (UTC) | Claim → cost | Org |
|---|---|---|---|---|---|---|
| cheap | `imports.ocr` | claude-haiku-4-5-20251001 | 2,167 / 246 | 2026-09-10 11:10:10.5 → 11:10:13.7 | 4p → 1p | tenant (founder test org) |
| mid | `insights.narrative` ×2 | claude-sonnet-5 | 1,920 / 141-142 | 2026-09-10 19:44:41.1 → 19:44:44.5 (and 21:35) | 3p → 1p | tenant (founder test org) |
| high | `hq.saga_decomposition` | claude-opus-5 | 271 / 599 | 2026-09-11 07:17:30.2 → 07:17:36.7 | 16p → 2p | **CrewFlow (Internal HQ)** |
| embedding | `memory.embedding_write` | openai · text-embedding-3-small | 601 / 0 | 2026-09-13 11:14 (settled) | 1p → 1p | **CrewFlow (Internal HQ)** |

Per-tier invariants verified on the rows: reservation created BEFORE provider execution; success=true, error_code null; settlement ≤ claim; reservation `settled` and linked to its invocation; content_hash present; **estate-wide unreconciled reservations = 0**; no tenant org charged for HQ work (internal-HQ attribution). The high proof additionally closed the full application loop: output NOT truncated (599 ≪ the 3,000 cap; the seam refuses `stop_reason=max_tokens` before parsing, so persistence itself certifies normal completion) → JSON parsed → schema + `validateStepGraph` passed → saga `eb3ffb7e` persisted with 8 dependency-linked steps under `template_key=__ai_assisted__` → `saga.created` audit row with `decomposition:"ai_assisted"`. No external side effect on any proof (founder/internal surfaces only).

EMBEDDING tier (armed + activated 2026-09-11→13, CEO-approved vendor: OpenAI): three-switch activation proven in order — binding merged dark (PR #869, nothing lit), key added (behavioural cron proof: telemetry `enabled:false, costUsd:0` — configured, flag holding), flag flipped via service-role SQL → full 6-row corpus backfilled in one tick (6/6 vectors, 0 failed, version `openai:text-embedding-3-small:d1536:v1`, dim 1536 matching the shipped pgvector column). Governed ledger row: 601 input tokens (vendor truth), 1p claim → 1p settle, linked, internal-HQ attributed; 6 `hq_embedding_runs`; 0 unreconciled estate-wide; HNSW ANN searchability proven by SQL probe (self-distance 0.0000, semantically-ordered neighbours). Real vendor cost ≈ $0.000012. Prerequisites shipped first: settings blob-preservation (#865), E2 real purge semantics (#868, migration 20261228), observability row (#867). Kill switches live: flag (instant), unbind (diff), key removal.

TRANSCRIPTION tier (armed 2026-09-13, CEO-approved vendor: OpenAI gpt-4o-mini-transcribe): **BUILT + DEPLOYED + CONFIGURED** — governor cost binding + transport binding shipped in one reviewed diff (PR #871, prod `9785c8e`); credential doctrine live (dedicated `TRANSCRIPTION_API_KEY` override OR the deployed `OPENAI_API_KEY`, openai-binding only); duration-as-token-proxy arithmetic and the 300s envelope pinned in `__tests__/ai/tier-bindings.test.ts`. Classification: **APPLICATION-PATH BLOCKED BY WHATSAPP/META** — the only channel that produces voice notes stays dark pending Meta activation, so the sole reachable surface is the super-admin provider self-test on /admin/ai-costs, **pending its first click**. Deliberately NOT claimed production-proven: no governed ledger row exists yet; this paragraph upgrades to a proof row when the self-test settles one.

Incident record (both root causes fixed + regression-locked): attempt 1 — `CREWFLOW_INTERNAL_ORG_ID` named an erased org → fail-closed FK refusal with zero footprint (fixed: dedicated `CrewFlow (Internal HQ)` org + env repoint; launch-readiness now checks the org EXISTS, red on set-but-missing; per-stage failure taxonomy shipped in PR #862). Attempt 2 — Opus output truncated at the seam's 1,500-token cap (ledger: output_tokens == cap) → honest parse refusal (fixed: cap 3,000 pinned ≤ the 3,200 envelope, vendor stop_reason threaded and truncation refused before parse; PR #863). Failure-control evidence banked along the way: two real fail-closed refusals with zero cost and zero stranded state, plus honest per-stage degradation.

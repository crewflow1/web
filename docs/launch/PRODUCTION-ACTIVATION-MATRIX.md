...
|---|---|---|---|
| **Generative AI estate** (~13 surfaces: quote writer, OCR, receptionist drafting, HQ narratives/seams, insights…) | R039 R050 R089 R091 R093 R101 (+R034) | Per-token usage; £100/org/month fail-closed ceiling with atomic pre-reservation | **ARMED 2026-09-10** (CEO-approved reviewed diff): cheap=claude-haiku-4-5-20251001, mid=claude-sonnet-5, high=claude-opus-5; `ANTHROPIC_API_KEY` now reaches the governed doors only (still 0 ungoverned entry points, security-pinned). Embedding + transcription tiers remain null — each is its own future reviewed diff (transcription additionally needs a transport diff). **CEO HOLD (2026-09-10): portal-chat generative auto-reply stays off** via the `CHAT_AUTO_REPLY_GENERATIVE=false` build constant (server/services/chat-auto-reply.ts) — the only surface that would auto-post unreviewed AI prose to a customer; every other customer-facing generative surface is draft-first/human-review. **LIVE — PRODUCTION-PROVEN, ALL THREE TIERS (2026-09-11)** — evidence in §AI-estate proof below |
...
## AI-estate production proof — all three tiers (stamped 2026-09-11)

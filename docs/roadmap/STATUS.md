...
**Production `main`:** `cf3a0fb` — verified against `/api/health`, not inferred
**Production migration tip:** `20261101` — read from `supabase_migrations.schema_migrations`, NOT inferred
...
| `20261102`–`20261104` | ALLOCATED to in-flight Wave-11 trains (van-stock `102`, telematics dark `103`, HQ workflow-saga `104`) — rebase onto latest main before migrate-first (they predate `101`) | in-flight |
| `20261105+` | **NEXT FREE** | unallocated — re-verify against the DB before claiming |

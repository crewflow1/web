...
**Production `main`:** `f99e3e5` — verified against `/api/health`, not inferred
**Production migration tip:** `20261098` — read from `supabase_migrations.schema_migrations`, NOT inferred
...
| `20261099`–`20261101` | ALLOCATED to in-flight Wave-10 trains (HMRC dark `099`, banking dark `100`, offline vertical-fill `101`) — NOT yet applied to prod; applied migrate-first in slot order at merge | in-flight |
| `20261102+` | **NEXT FREE** | unallocated — re-verify against the DB before claiming |

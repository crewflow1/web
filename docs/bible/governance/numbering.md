...
migration)** review the **Compatibility Layer Rule**; and on the **LR4 (production-confidence
audit)** review the **Retirement Readiness Rule** (all ten homed in
...
confidence audit — removing nothing; and **LR5** (design only: the compatibility-retirement sequence,
the rollback-retirement sequence, the legacy-column removal order, cleanup validation, and final
migration-completion criteria — the **Retirement Readiness Rule**) is **authorised as a proposal**.
Legacy storage, rollback and parity tooling remain in place until later, separately-authorised removal
phases.

...
memory scope)** review the **Registry Completeness Rule**; and on the **LR3 (runtime read-path
migration)** review the **Compatibility Layer Rule** (all nine homed in
...
registry — the **Registry Completeness Rule**) is **merged** (PR #243); and **LR4** (demonstrate
...
**Compatibility Layer Rule**) is **in progress (evidence-gathering)**: a read-only
production-confidence audit measures the §4 window, and the [LR4 production-confidence evidence
ledger](./directive-015-lr4-production-confidence-evidence.md) records the Compatibility Layer
Rule's four attributes made concrete — removing nothing. Legacy storage, rollback and parity
tooling remain in place until later, separately-authorised removal phases.

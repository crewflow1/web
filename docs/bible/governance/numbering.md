...
the **Data Removal Rule**; on the **LR5.4 (legacy authority column removal)** review the **Hidden
Read Path Rule**; and on the **LR5.4A (migrate the last hidden SQL reader onto the registry)** review
the **Legacy Independence Rule** (all sixteen homed in
...
already in every grant), no schema deletion, independently validated, now **merged** (PR #256) — and **LR5.4B** then performs
...
On the **LR5.4A** review the CEO accepted the migration, set the **Legacy Independence Rule** (a
physical legacy structure may be removed only once it is completely independent of runtime execution —
no write, read, hidden-SQL, rollback or operational dependency — with that independence independently
demonstrated and reviewed) and authorised **LR5.4B** as the **final implementation increment**: drop
`ai_employees.tools_allowed` / `permissions`; remove the obsolete compatibility helpers, the parity
oracle, the mirror infrastructure and the migration scaffolding; re-point the runtime fail-safe off the
columns to a default-deny floor; and preserve the migration, audit, confidence and governance
histories — physical removal only, no behavioural change. Directive #015 is declared complete only
after LR5.4B's final six-gate validation and an independent review.

...
| 3 | **AI SDK** | **Partial** | The **Memory facet** (`server/sdk/memory.ts` — `createMemory` / `BoundMemory`), plus the **events** and **comms** facets, the **output envelope** (`server/sdk/{events,comms,output}.ts`) and the runner's **evidence-drain** — shipped under **#014 Phase A** (**ADR 0008**; PRs #208→#209→#210 on the `#011` integration branch) — and the **permission doorman + P4**: the pure gate `server/sdk/gate.ts` (`evaluateAction → GateVerdict`) consumed by `ctx.proposeActions` in the runner, with `resolveEmployeePosture` and the `ai.action_permitted` audit verb — shipped under **#014 Phase B** (**ADR 0008** Decisions 4 & 8). The remaining envelope (the typed tool registry · the API gateway + cost metering — Phases C–D) is **not yet written**; contract #3 stays **Partial** until the full envelope lands, then graduates **Partial → Established**. The facet/enforcement standards introduced on the Phase A/B reviews — the **Facet Isolation Rule** and its generalisation the **Runtime Composition Rule** (the runtime is the only composer of kernel capabilities; no facet orchestrates another), the *facets-expose / runtime-composes* principle, and the **Policy vs Mechanism Rule** (the gate is pure policy; the runtime supplies mechanism) — are homed in the [Kernel Contract Map](./kernel-contract-map.md) (§2 / §4.2). Owned by the AI SDK directive (D-04 / **#014**). |

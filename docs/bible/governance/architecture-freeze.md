...
| 3 | **AI SDK** | **Established** *(executor rollout deferred)* | The **Memory facet** (`server/sdk/memory.ts` — `createMemory` / `BoundMemory`), plus the **events** and **comms** facets, the **output envelope** (`server/sdk/{events,comms,output}.ts`) and the runner's **evidence-drain** — shipped under **#014 Phase A** (**ADR 0008**; PRs #208→#209→#210 on the `#011` integration branch) — and the **permission doorman + P4**: the pure gate `server/sdk/gate.ts` (`evaluateAction → GateVerdict`) consumed by `ctx.proposeActions` in the runner, with `resolveEmployeePosture` and the `ai.action_permitted` audit verb — shipped under **#014 Phase B** (**ADR 0008** Decisions 4 & 8). The remaining envelope was built in small reviewable **Phase C** increments **C1 → C2 → C3 → C4** (authorised on the acceptance of **ADR 0009** — the SDK executor + apply-on-approval runtime; CEO independent CTO review, 2026-06-28): **C1**, the **typed tool registry contract** (`server/sdk/tools.ts`, descriptive only), **C2**, the **executor contract** (`server/sdk/executor.ts` — registry consumption + the execution boundary, descriptive/dependency-injected only, no runner wiring), **C3**, the **apply-on-approval marker** (`server/sdk/application.ts` — the deterministic idempotency key, the separate application record, and `applyOnce`, dependency-injected only, no runner wiring or migration), and **C4**, the executor **Reference Path** (`__tests__/sdk/reference-path-execution.test.ts` — the real registry → gate → executor → application composed end-to-end with no mock, proving apply, replay, idempotency, failure-recovery and escalation), are **all shipped and merged** (PRs #219, #221, #223; the Application Atomicity Rule docs #224; and the Reference Path #225). **C4 completed Phase C, and Phase C completes Directive #014** (CEO review, 2026-06-28). Contract #3 therefore graduates **Partial → Established** per its pre-defined trigger. The **executor rollout** (wiring the proven executor into the live run loop) and the **API gateway + cost metering** (originally envisaged as Phase D) are a deferred future **extension** of the now-established contract — governed by the §2 SDK Stability + ABI rules (*extend before replace*), not a precondition of its establishment; **not yet written**. The facet/enforcement standards introduced across the Phase A/B reviews and **Phase C** (the acceptance of **ADR 0009** and the **C1**, **C2**, **C3** and **C4** reviews) — the **Facet Isolation Rule** and its generalisation the **Runtime Composition Rule** (the runtime is the only composer of kernel capabilities; no facet orchestrates another), the *facets-expose / runtime-composes* principle, the **Policy vs Mechanism Rule** (the gate is pure policy; the runtime supplies mechanism), the **Executor Boundary Rule** (the executor applies only gate-cleared actions and is mechanism only — policy stays with the gate, approval with the Approval Engine, lifecycle with the Task Engine), the **Registry Immutability Rule** (tool definitions are immutable platform metadata, registered at initialisation; runtime execution consumes the registry and never mutates it), the **Executor Idempotency Rule** (every executor-applied action is idempotent by design or protected by a deterministic idempotency key — derived from stable execution identity; the executor never relies on "probably once"), and the **Application Atomicity Rule** (a successful application record represents exactly one completed application — never written before successful execution, never showing a failed execution as applied; ambiguous persistence must fail rather than record a false state), the **Reference Path Rule** (every frozen-contract extension keeps one canonical end-to-end path green — the living regression gate), and — set on the **C4** review — the **Reference Implementation Rule** (every new kernel capability has exactly one canonical reference implementation that exercises the complete intended lifecycle, proven before the platform expands on top of it) — are homed in the [Kernel Contract Map](./kernel-contract-map.md) (§2 / §4.2 / §4.4). Owned by the AI SDK directive (D-04 / **#014**), which is **complete**. |
...
| 8 | **Capability Registry** | **Established** *(cutover CEO-gated)* | One declarative authority model — the `hq_capabilities` catalogue (`supabase/migrations/20260806000000_capability_registry.sql`) + the scoped `hq_capability_grants` table (`global ⊇ organization ⊇ department ⊇ employee`) — resolved at runtime by a **pure, total** composition law (`server/sdk/registry-resolver.ts` — tokens UNION, `can_execute` **deny-wins**, `requires_approval` the **approval ratchet**, `budget_default` **effective-minimum**, `memory_scope` **most-specific**) over a **default-deny floor**, served by the IO bridge `server/sdk/registry-parity.ts` with the floor as the automatic fail-safe; **ADR 0010**. Built and switched under **#015 / D-05** (R1 schema → R2 backfill + parity → R3 pure-resolver shadow → R4 authority switch), then the **legacy-removal phase** migrated every write + read path (application *and* database), banked production confidence (`server/sdk/registry-confidence.ts`), retired the mirror / rollback / parity scaffolding, and **dropped the scattered legacy columns** `ai_employees.tools_allowed` + `permissions` (`supabase/migrations/20260812000000_lr5_4b_remove_legacy_authority_columns.sql`, LR5.4B / PR #258) — preserving `memory_scope` / `department`. The registry is now the **sole runtime authority**; no runtime surface reads the removed columns, and the default-deny floor is the fail-safe. Graduated **Reserved → Established** (CEO completion review). Cutover to `main` + the (irreversible) production migration are CEO-gated. Completion record: [`directive-015-completion-report.md`](./directive-015-completion-report.md). |
...
- **Numbering:** directive/ADR/volume numbers are governed by
  [`numbering.md`](./numbering.md). ADRs referenced here (0001–0010) live in
  [`../decisions/`](../decisions/); the next free ADR number is **0011** (ADR `0010`, the
  Capability Registry under D-05 / #015, is **Accepted**; with the legacy-removal phase merged
  through LR5.4B, contract #8 has **graduated Reserved → Established** and Directive #015 is
  **complete** — its permanent record is [`directive-015-completion-report.md`](./directive-015-completion-report.md)).

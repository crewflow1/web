...
| 3 | **AI SDK** | **Partial** | The **Memory facet** (`server/sdk/memory.ts` — `createMemory` / `BoundMemory`), plus the **events** and **comms** facets, the **output envelope** (`server/sdk/{events,comms,output}.ts`) and the runner's **evidence-drain** — shipped under **#014 Phase A** (**ADR 0008**; PRs #208→#209→#210 on the `#011` integration branch). The remaining envelope (the permission doorman + P4 · the typed tool registry · the API gateway + cost metering — Phases B–D) is **not yet written**; contract #3 stays **Partial** until the full envelope lands, then graduates **Partial → Established**. The two facet-composition standards introduced on the Phase A review — the **Facet Isolation Rule** and the *facets-expose / runtime-composes* principle — are homed in the [Kernel Contract Map](./kernel-contract-map.md) (§2 / §4.2). Owned by the AI SDK directive (D-04 / **#014**). |

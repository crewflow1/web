...
| 12 | **KPIs** | XIII §20 metrics, §19 cost | accuracy, latency, revenue, hours saved, CSAT, approval rate, failure rate, escalation rate, execution cost, ROI, quality score. **Measured set (recorded reading, 2026-08-30):** what is HONESTLY measurable today is persisted per employee/month in `ai_employee_kpis` — tasks completed/failed, failure rate, approvals requested, attributed cost (`ai_invocations.ai_employee_id`). Revenue / hours-saved / CSAT / ROI have NO real data source yet (no closed-loop revenue attribution, no time-baseline, no survey instrument) and the framework forbids invented figures — those columns join the KPI row when a genuine source exists, not before. |

...
> ## 🔄 RC2 REFRESH — release candidate through PR #374 (2026-07-19)
>
> **This manifest was authored 2026-07-17 for the directive→main baseline. This RC2
> header refreshes it to the CURRENT candidate**, which adds the Site Management +
> Site Reports + Asset Management programme (PRs **#367–#374**) on top of that
> baseline. Consolidated on branch `release/rc2-site-management-assets` (= the
> verified stack tip). **Supersedes PR #363** (which predates #367–#374). Still:
> **do not merge, do not deploy** — this is a CEO go/no-go checkpoint.
>
> **Added since the 2026-07-17 baseline (all additive, all dark until routed, each
> individually CI-green):**
>
> | PR | Feature | Migration | New routes |
> |----|---------|-----------|-----------|
> | #367 | Snagging | `20260919` | `/snags` |
> | #368 | Daily Site Diary | `20260920` | `/diary` |
> | #369 | Toolbox Talks / RAMS | `20260921` | `/toolbox` |
> | #370 | Site Reports (core, immutable snapshot + state machine) | `20260922` | `/site-reports` |
> | #371 | Site Reports PDF | — (no migration) | `/api/site-reports/[id]/pdf` |
> | #372 | Site Reports customer-portal delivery | `20260923` | `/customer-portal/[token]/reports…` |
> | #373 | Asset register | `20260924` | `/assets` |
> | #374 | Asset assignment & custody | `20260925` | (asset detail custody) |
>
> **7 additive migrations (`20260919`→`20260925`), applied in filename order.** None
> is destructive; none rewrites historical data; the only irreversible migration in
> the whole release remains **LR5.4B (`20260812`)** from the original baseline —
> handled exactly as this manifest already documents (deploy code first, then push).
>
> **Shared `tenant_attachments.target_table` CHECK — final consolidated state**
> (proven applied by #374's real-Postgres integration gate over the full chain):
> `customers, jobs, quotes, invoices, suppliers, memberships, leads, snags,
> site_diary_entries, toolbox_talks, site_reports, assets, asset_assignments` —
> and unsupported values remain rejected.
>
> **External requirements for #367–#374: NONE (verified).** No new env vars, no new
> vendors, no new feature flags, no new crons, no new storage buckets (all reuse the
> existing `tenant-attachments` bucket + pipeline). QR (M3) would add the `qrcode`
> dependency — **not in this release**.
>
> **CI:** every one of #367–#374 passed all 8 gates independently; the RC2 PR re-runs
> the full authoritative suite (incl. the full-chain fresh-DB migration apply) against
> `main`. Per-suite execution evidence is quoted in the RC2 report.
>
> **Deployment order (unchanged principle):** deploy code (Vercel merge to `main`)
> **first**, then `supabase db push` the 7 additive migrations. Rollback: the new
> features are additive + unrouted-from-nav-only; a code revert hides them, and the
> additive tables/columns can remain or be dropped (reversible) with no data loss.
>
> _Full baseline detail (the 54-migration directive, LR5.4B handling, comms readiness,
> health checks) is unchanged below._
>
> ---

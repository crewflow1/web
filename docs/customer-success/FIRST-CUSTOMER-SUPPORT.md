...
| **"Worker can't see today's job"** | L1: check the worker has a **rota entry today** (Staff → Rota) — Today's rota on My Day is the worker's main pointer; also check job `assigned_to` + status ≠ cancelled and membership exists. The "My jobs" panel lists their own assignments (fixed + regression-tested 2026-08-29); rota and the Jobs tab are additional paths. | 10 min |
...
**Numbers we can honestly state:** schema-restore RTO ≈ 30 s · app redeploy RTO ≈ 5 min · **data RPO today ≤ 24 h — IF daily backups exist for this plan tier (dashboard-unconfirmed; the 2-min check below settles it)**; WAL archiving verified healthy, 0 failures · with PITR enabled RPO ≈ 2 min · **data-restore RTO: not yet measured** (dashboard operation; rehearse when the actions below land). No other SLA is promised.

## Pre-customer #1 operational checklist

- [ ] **FIRST (2 min): open Supabase dashboard → Database → Backups and confirm the plan tier + a visible daily backup list** — this converts the RPO claim from assumption to fact (or falsifies it; a tier without daily backups means NO data safety net until PITR/dumps land).

...
| **4. Site Management** | ◑ largely shipped | Snagging #367 · Daily Diary #368 · Toolbox Talks #369 · Site Reports #370–#372 (immutable, PDF, portal). **Toolbox Talks evolved into evidence-grade H&S** (wave `20261025–28`, branch `feat/health-safety-toolbox-talks`): draft→deliver→acknowledge→revise lifecycle, shared acknowledgement engine (Tier A authenticated vs Tier B recorded), atomic revisions + re-ack, frozen snapshot + PDF, append-only signed-sheet evidence, job-hub + dashboard signals, WCAG 2.2 AA — CI-green locally, **unmerged/undeployed** (see `docs/health-safety/toolbox-talks.md`). Progress-photo galleries + weather intelligence (external API, gated) remain. |

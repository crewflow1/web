...
**6. The Bible is a living knowledge base (lessons are mandatory, not optional).** Architecture-before-code keeps the spec ahead of the *build*; this rule keeps it ahead of *experience*. Every engineering lesson discovered in **production, CI, QA, customer feedback, security testing, or a postmortem** is recorded in [Ch.20 §20.6](20-glossary-conventions-decision-log.md) — as a permanent entry, in the *same* change that resolves it — and, wherever possible, hardened so the failure class becomes *impossible to repeat* (made uncompilable, CI-gated, or principle-bound). A fix that lands without its lesson is incomplete. This is how the Bible compounds: it grows not only by **directive** (new capability) and by **deviation** (the design was wrong), but by **evidence** (the system taught us something). Recorded as [ADR-016](20-glossary-conventions-decision-log.md). *(CEO Directive, post-OQ-16.)*
...
>
> **The test for any incident or discovery:** *Is the lesson in §20.6, and is the failure class now impossible to repeat?* If yes, the loop is closed. If no, the work is unfinished.

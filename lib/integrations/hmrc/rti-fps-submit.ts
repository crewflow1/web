...
import type { FpsReturn } from "./rti-fps";

/**
 * HMRC RTI — the Full Payment Submission (FPS) SUBMIT adapter: the provider HTTP
 * half of filing a prepared FPS. DARK / recognition-gated.
 *
 * This is the ONLY code in the substrate that POSTs an RTI Full Payment Submission
 * to HMRC. It takes an already-composed FPS payload (lib/integrations/hmrc/
 * rti-fps.ts), the employer's HMRC handle (the PAYE reference the scheme files
 * under), an access/refresh token pair (decrypted by the orchestrator immediately
 * before the call) and the mandatory fraud-prevention headers, and POSTs to HMRC's
 * RTI "full payment submission" endpoint. On a 401 it refreshes the access token
 * once and retries. It returns HMRC's receipt (processingDate / submissionReference)
 * on success, or a classified failure.
 *
 * It mirrors lib/integrations/hmrc/vat-submit.ts EXACTLY — the same dark guard, the
 * same 401→refresh→retry, the same terminal/rejected/transient classification —
 * differing only in the endpoint, the payload type, and the receipt vocabulary (an
 * RTI async submission answers with a correlation id / receipt reference rather
 * than a VAT form-bundle number).
 *
 * ── DARK BY DEFAULT ─────────────────────────────────────────────────────────
 * `submitFpsReturnToHmrc` REFUSES (returns not_configured, NO `fetch`) when HMRC
 ...
 * Even once connectable, HMRC rejects an RTI submission from software it has not
 ...
 * this module's own guard both refuse). The exact RTI endpoint path and envelope
 ...
 *   rejected  — HMRC answered a BUSINESS validation failure (400 / 403 with an
 *               RTI error body: bad pay date, PAYE-ref mismatch, duplicate FPS).
...
/** HMRC RTI production base. The submit endpoint is `/{employerRef}/rti/full-payment-submissions`. */
const PAYE_API_BASE = "https://api.service.hmrc.gov.uk/organisations/paye";

/**
 * HMRC's success receipt for a submitted FPS. Every field is optional per the RTI
 * contract. `submissionReference` is HMRC's async submission handle (a correlation
 * id) — the RTI analogue of a VAT form-bundle number.
 */
export type HmrcFpsReceipt = {
  /** ISO 8601 timestamp HMRC recorded the submission. */
  processingDate: string | null;
  /** HMRC's unique submission reference (correlation id) for this FPS. */
  submissionReference: string | null;
};

type RefreshedTokens = { accessToken: string; refreshToken: string | null; expiresAt: string | null };

export type FpsSubmitResult =
  | {
      ok: true;
      receipt: HmrcFpsReceipt;
...
/**
 * POST a prepared FPS to HMRC's RTI submit endpoint. Dark-gated: refuses (no fetch)
 * unless HMRC is connectable. On a 401 the stored refresh token renews the access
 * token and the request is retried once; the refreshed tokens are returned so the
 * orchestrator can persist them.
 */
export async function submitFpsReturnToHmrc(params: {
  /** The employer's HMRC handle — the PAYE reference the scheme files under. */
  employerRef: string;
  payload: FpsReturn;
  tokens: { accessToken: string; refreshToken: string | null };
  fraudHeaders: Record<string, string>;
}): Promise<FpsSubmitResult> {
  const { employerRef, payload, tokens, fraudHeaders } = params;
  ...
  if (!isHmrcConnectable()) {
    return {
      ...
      message: "HMRC is not configured; no FPS was submitted.",
    };
  }

  const url = `${PAYE_API_BASE}/${encodeURIComponent(employerRef)}/rti/full-payment-submissions`;
  ...
  try {
    ...
    return {
      ok: false,
      reason: "error",
      message: `fps submit request failed: ${e instanceof Error ? e.message : "network error"}`,
      ...(refreshed ? { refreshed } : {}),
    };
  }
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) {
      return {
        ok: false,
        reason: "error",
        message: "fps submit returned 401 after token refresh; re-consent required",
        terminal: true,
        ...(refreshed ? { refreshed } : {}),
      };
    }
    // 400 / 403 ⇒ HMRC BUSINESS rejection of the return (bad pay date, PAYE-ref
    // mismatch, duplicate FPS, malformed lines). No retry helps — the orchestrator
    // records status='rejected' with this error.
    if (res.status === 400 || res.status === 403) {
      return {
        ok: false,
        reason: "error",
        message: `fps submit rejected by HMRC (${res.status})`,
        rejected: true,
        ...(refreshed ? { refreshed } : {}),
      };
    }
    ...
    return {
      ok: false,
      reason: "error",
      message: `fps submit returned ${res.status}`,
      ...(refreshed ? { refreshed } : {}),
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      reason: "error",
      message: "fps submit returned an unreadable receipt body",
      ...(refreshed ? { refreshed } : {}),
    };
  }

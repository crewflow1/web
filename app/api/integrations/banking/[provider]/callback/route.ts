...
/**
 * Banking OAuth — CONNECT callback. DARK.
 *
 ...
 *
 * ── FCA LEGAL BOUNDARY ──────────────────────────────────────────────────────
 * Account Information Services are FCA-regulated. This route can never complete a
 * live bank connection before AISP authorisation exists — the dark short-circuit
 * below is the technical guarantee.
 *
 * DARK INVARIANT. `isBankingProviderConnectable` is false today (no credentials,
 * no bound provider, flag off), so this route short-circuits BEFORE
 ...
 * unreachable dark. Tokens, once live, are AES-256-GCM encrypted application-side
 * before they reach the DB (the token-crypto tripwire enforces this); this build
 ...
 * the 600s cookie window, so both are expired (maxAge 0) on every response that
 * leaves this route after the dark guard.
 */
...
export async function GET(
  ...
  // aggregator credentials + the feature flag + FCA authorisation.
  if (!isBankingProviderConnectable(provider)) {
    return NextResponse.json(
      {
        ok: false,
        provider,
        status: "not_configured",
        message: `${provider} bank feed is not configured; the connection was not completed.`,
      },
      { status: 200 },
    );
  }
  ...
  // exchange. Dark today (no creds ⇒ never connectable), so this never triggers.
  if (!isTokenEncryptionConfigured()) {
    console.error("[banking] refusing exchange: token encryption key missing", { provider });
    return NextResponse.json(
      {
        ok: false,
        provider,
  ...
  // The aggregator connection reference is carried on the callback query
  // (TrueLayer connection id / Plaid item / Nordigen requisition ref).
  const connectionRef = searchParams.get("ref") ?? searchParams.get("requisition_id");
  ...
  const redirectUri = `${origin}/api/integrations/banking/${provider}/callback`;
  ...
  const handle = exchanged.tokens.connectionRef;
  if (!handle) {
  ...
  // bank_connections post-dates the generated types.ts; cast to a minimal upsert
  ...
  const { error } = await loose.from("bank_connections").upsert(
    {
      org_id: ctx.org.id,
      provider,
      status: "connected",
      connection_ref: exchanged.tokens.connectionRef,
      institution_id: exchanged.tokens.institutionId,
      institution_name: exchanged.tokens.institutionName,
  ...
  if (error) {
    ...
    console.error("[banking] connection upsert failed", { provider, message: error.message });
    return backToIntegrations(origin, "error", provider);
  }

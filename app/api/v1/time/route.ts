...
import {
  TIME_DTO_SELECT,
  toPublicTimeEntryDto,
  type TimeEntryRowForDto,
} from "@/lib/public-api/time";
import { parsePagination, rangeFor } from "@/lib/public-api/jobs";

/**
 * GET /api/v1/time — the public, key-authenticated TIME-ENTRIES READ.
 *
 ...
 *   - 403 without the read:time scope.
 *   - 429 over the api_v1 budget (120/min, keyed by KEY ID).
 *   - 200 → { data: PublicTimeEntryDto[], pagination: { page, per_page, has_more } }.
 *
 * SECURITY: the read is pinned to key.orgId (never a client-supplied org), the
 * projection is an EXPLICIT allowlist — the shift window only. Worker identity
 * (user_id), precise GPS location, the linked job, notes and payroll linkage
 * are all excluded (see lib/public-api/time.ts). Page size is bounded, order
 * stable.
 */
      ...
      order: (
        ...
        order: (
          ...
          range: (
            from: number,
            to: number,
          ) => Promise<{ data: TimeEntryRowForDto[] | null; error: { message?: string | null } | null }>;
        };
      };
    };
  };
...
export async function GET(request: Request): Promise<Response> {
  const guard = await guardPublicApiRequest(request, "read:time");
  ...
  const { data, error } = await (
    admin.from("time_entries") as unknown as TimeListRead
  )
    .select(TIME_DTO_SELECT)
  ...
  if (error) throw readFailure("public-api: time list", error);

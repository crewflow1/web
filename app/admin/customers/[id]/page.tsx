...
export default async function HqCustomerDetailPage({
  ...
  const { org, owner, members, subscription, ltv, health, timeline, demoRequest } =
    snap;
  ...
  return (
    <div className="space-y-6">
              ...
              {org.tos_accepted_at ? (
                <>
                  <span className="font-medium text-slate-700">
                    {org.tos_version ?? "unknown version"}
                  </span>{" "}
                  · accepted {org.tos_accepted_at.slice(0, 10)}
                  {org.tos_version === "legacy"
                    ? " (backfilled from signup date)"
                    : ""}
                </>
              ) : (
                "— not recorded"
      ...
      {/* Users — the full member roster (identity + role only, never pay) */}
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <header className="border-b border-slate-200 px-5 py-3">
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Every member of this workspace — no impersonation needed. Name,
            email, role and join date only; pay data is never shown here.
          </p>
        </header>
        {members.length === 0 ? (

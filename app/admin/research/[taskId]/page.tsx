...
import { BackToResearch, ProvenanceBadge } from "../_components";
import { LiveRun } from "./_live";
import { ResearchReport } from "./_report";

/**
 * Research AI — single run view (CEO Directive 005).
 *
 * One URL for the whole lifecycle of a research task. While the run is in
 * flight it renders the live, polling checklist; the moment it finishes it
 * renders the full intelligence report from the same task row. The client
 * view refreshes this route on completion, so the hand-off is seamless.
 */
...
export default async function ResearchRunPage({
  ...
  const report = terminal ? await getResearchReport(taskId) : null;
  const provenance = report?.provenance ?? state.summary?.generatedBy ?? null;
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 text-slate-100 shadow-xl">
      ...
      <div className="relative border-b border-slate-800 px-5 py-6 sm:px-7">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold tracking-tight text-white">
                {state.companyName ?? "Researching…"}
              </h1>
              <p className="mt-0.5 text-sm text-slate-400">
                Research AI · {state.companyId ? "company intelligence report" : "new prospect"}
              </p>
            </div>
          </div>
        ...
        <div className="relative mt-4 inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1 text-[11px] font-medium text-emerald-300 ring-1 ring-inset ring-emerald-400/30">
          ...
          Read + draft only · every step audited · no autonomous outreach
        </div>
      </div>

      {/* Body — live while running, report when finished */}

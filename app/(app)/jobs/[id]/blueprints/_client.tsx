 ...
 *
 * Both upload forms post through <StateForm> (FormState + full-document
 * navigation) — see components/forms/StateForm.tsx for why `redirect()` is
 * unsafe at this depth.
 */
...
export function AddDrawingForm({ jobId }: { jobId: string }) {
  ...
  return (
    <StateForm action={uploadBlueprint.bind(null, jobId)} className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
...
export function AddRevisionForm({ jobId, blueprintId }: { jobId: string; blueprintId: string }) {
  ...
  return (
    <StateForm action={uploadRevision.bind(null, jobId, blueprintId)} className="mt-3 grid gap-3 rounded-md border border-slate-200 bg-slate-50 p-3 sm:grid-cols-3">

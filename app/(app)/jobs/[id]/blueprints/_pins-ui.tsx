...
import {
  placeSnagPinAction, placeNotePinAction, linkSnagPinAction, deletePinAction,
} from "./pin-actions";
import {
  normalizedToPercent, pinDisplayStatus, pinShortLabel,
  type BlueprintPin, type PinTone,
...
export function PinMarker({ pin, active, onActivate }: { pin: BlueprintPin; active: boolean; onActivate: (id: string) => void }) {
  ...
  const glyph = pin.kind === "note" ? "✎" : "!";
  return (
    <button
      type="button"
      onClick={(e) => { e.stopPropagation(); onActivate(pin.id); }}
      aria-label={`${pin.kind === "note" ? "Note" : "Snag"} pin: ${pinShortLabel(pin)} (${label})`}
...
export function PlacementSheet({
  jobId, versionId, page, u, v, linkable, onDone, onCancel,
}: {
  jobId: string; versionId: string; page: number; u: number; v: number;
  linkable: Linkable[]; onDone: () => void; onCancel: () => void;
}) {
  const [tab, setTab] = useState<"snag" | "note" | "link">("snag");
    ...
    <div role="dialog" aria-label="Add a pin" className="pointer-events-auto fixed inset-x-0 bottom-0 z-40 mx-auto max-w-lg rounded-t-2xl border border-slate-200 bg-white p-4 shadow-2xl sm:bottom-4 sm:rounded-2xl">
      ...
      <div className="mt-3 flex gap-2">
        <button type="button" onClick={submit} disabled={busy} className="min-h-[44px] flex-1 rounded-md bg-slate-900 px-4 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-50">
          {busy ? "Saving…" : tab === "note" ? "Add note" : tab === "link" ? "Link snag" : "Create snag"}
        </button>
...
export function PinDetail({
  pin, jobId, canDelete, onClose, onChanged,
}: { pin: BlueprintPin; jobId: string; canDelete: boolean; onClose: () => void; onChanged: () => void }) {
  const [busy, setBusy] = useState(false);
  const { label, tone } = pinDisplayStatus(pin);
  return (
    <div role="dialog" aria-label="Pin details" className="pointer-events-auto fixed inset-x-0 bottom-0 z-40 mx-auto max-w-lg rounded-t-2xl border border-slate-200 bg-white p-4 shadow-2xl sm:bottom-4 sm:rounded-2xl">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {pin.kind === "note" && pin.note ? <p className="mt-1 whitespace-pre-wrap text-sm text-slate-600">{pin.note}</p> : null}
        </div>
...
export function FilterChips({
  value, onChange,
}: { value: { kind: "all" | "snag" | "note"; status: "all" | "open" | "resolved" }; onChange: (v: { kind: "all" | "snag" | "note"; status: "all" | "open" | "resolved" }) => void }) {

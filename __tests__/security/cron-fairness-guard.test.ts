...
const ALLOWLIST: Record<string, string> = {
  ...
  "hq-exec-tick":
    "Per deterministic executive runner (the 13 execs — ceo/cto/cfo/coo/sales/marketing/product/qa/finance/operations/support/customer_success/exec_assistant): one idempotent per-DAY enqueue (dedupeKey collapses a re-tick) then a claim-ONE-and-exit drain off the atomic FOR UPDATE SKIP LOCKED task lease (limit 1 each); a claimed task takes a lease so overlapping passes never re-serve it. Each handler is a single internal board fold (no external per-item I/O, no model call). Same lease-claim class as hq-runners-tick / research-drain.",
  "hq-roster-workers-tick":
    "Per deterministic roster-worker runner (the 12 identity-only roles — security/devops/database/api/documentation/onboarding/hr/legal_compliance/design/orchestrator/workflow/memory_manager): one idempotent per-DAY enqueue (dedupeKey collapses a re-tick) then a claim-ONE-and-exit drain off the atomic FOR UPDATE SKIP LOCKED task lease (limit 1 each); a claimed task takes a lease so overlapping passes never re-serve it. Each handler is a single internal bounded DB read + pure fold (no external per-item I/O, no model call). Same lease-claim class as hq-exec-tick / hq-runners-tick / research-drain.",
  ...
  "push-drain":
    "LIMIT-bounded (PUSH_DRAIN_BATCH_SIZE=50) drain of push_deliveries ordered by scheduled_for ASC; a settled row (sent/skipped) leaves the status='queued' candidate set and a transient failure reschedules via backoff (scheduled_for moves forward = recency rotation) — self-draining, exactly like notifications-drain. Dark-safe: with no VAPID it marks the batch skipped and sends nothing.",
  "sms-drain":
    "LIMIT-bounded (SMS_DRAIN_BATCH_SIZE=50) drain of sms_deliveries ordered by scheduled_for ASC; a settled row (sent/skipped) leaves the status='queued' candidate set and a transient failure reschedules via backoff (scheduled_for moves forward = recency rotation) — self-draining, exactly like push-drain and notifications-drain. Dark-safe: with no Twilio provider it marks the batch skipped and sends nothing.",

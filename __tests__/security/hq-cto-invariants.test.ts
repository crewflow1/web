...
/**
 * HQ CTO AI — trust-boundary invariants (hermetic source scan).
 *
 * Pins:
 ...
 *      files introduces an ungoverned inference entry point — no SDK
 ...
 *      prove the CTO surface contributes nothing to that count.
 */
...
const PAGE = read("app/admin/cto-ai/page.tsx");
const SERVICE = read("server/services/hq-cto.ts");
const PURE = read("lib/hq/cto.ts");
...
describe("CTO AI — super-admin gated (HQ-only)", () => {
...
describe("CTO AI — contributes NOTHING to the governance-closure ratchet", () => {
  ...
  it("no CTO file constructs a vendor SDK or reads a vendor credential", () => {
  ...
  it("no CTO file gates on a bare credential or opens a model door", () => {
    for (const [name, code] of CODES) {
  ...
  it("no CTO file registers a governor feature key (unwired permission = drift)", () => {
    for (const [name, code] of CODES) {
...
describe("CTO AI — the pure layer is clock- and Supabase-free", () => {
  it("takes an injected `now` and never touches Date.now or the DB", () => {
    expect(PURE).toMatch(/computeCtoBoard\([^)]*now:\s*Date/);

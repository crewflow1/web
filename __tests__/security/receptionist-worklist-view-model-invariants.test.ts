...
describe("receptionist worklist view model — it consumes only the Worklist Session", () => {
  ...
  it("with R58 in the tree, the R39 read surface is called by exactly the R40 route, the R58 attention queue + the R39 runtime", () => {
    ...
    // The R43 view model must STILL NOT appear: it reads THROUGH the session (which reads through the client,
    // which GETs the API), never by calling the read surface.

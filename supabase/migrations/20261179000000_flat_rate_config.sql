...
-- The COMPUTATION (rate resolution, first-year discount, effective-date gating, and
-- the flat box-1 calculation) lives in the single VAT authority (lib/tax/compute.ts
-- — resolveFlatRateForPeriod + computeVatQuarter's flatRate option). There is still
-- exactly one VAT calculator.
--
-- LIMITED-COST IS AN EXPLICIT DECLARATION ('yes'|'no'|'unset'), NEVER INFERRED.
-- Finance rows carry no goods/services split, so auto-deriving the limited-cost
-- test would risk applying the LOWER sector rate and UNDER-declaring VAT for a
-- services-heavy trader. The filed return uses the org's declaration; 'unset' (the
-- default) files the CONSERVATIVE 16.5% and the app prompts the org to declare.
--
-- The DB default is a DISABLED config, so FRS never applies and no existing org's
-- VAT figures move until it opts in. Deep shape validation is at the write boundary
-- (lib/org-config/schema.ts, zod); the DB CHECK only pins that the value is a jsonb
-- OBJECT so a scalar/array can never be stored.
--

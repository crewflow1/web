-- Fan out 'lead.created' activity rows into the notifications table so
-- demo-form submissions reach the bell. The original trigger from the
-- notifications migration listed quotes/invoices/time/payroll/etc but
-- omitted leads, which means a demo-booking lead would never notify
-- the owner. Adding `lead.%` to the org-wide branch.
